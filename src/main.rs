...
lazy_static! {
  static ref AUTH_RE: Regex = Regex::new(r"^\(([0-9]*)\).*Access-Request.*").unwrap();
  static ref ACC_RE: Regex = Regex::new(r"^\(([0-9]*)\).*Accounting-Request.*").unwrap();
...
fn main() {
  ...
  let matches = App::new("FreeRADIUS-debug-helper")
    .version("0.1.0")
    .author("Kris <kristhesavage@gmail.com>")
...
fn find_packet_type(input_line: &str, packet_type: RadiusPacket) -> Option<u32> {
...
fn find_packet_number(input_line: &str) -> Option<u32> {
  let packet_number: String = PACKET_NUM.captures(&input_line).and_then(get_first_group)?;
